...
**Live page:** <https://claude-models.agensse.com/>. It is served from `main`: the server checks `main` every 5 minutes and copies a whitelist of files (`index.html`, `raw-data.csv` and the root files listed under *Files*); nothing is built on the server, so `index.html` must be committed built, and a new file served at the root must be added to that whitelist. Exploratory blocks (value score, window tuner, full method) are collapsed by default; click to expand. To publish a fork, first set `SITE_URL` to its own address and remove `BING_SITE_VERIFICATION` and `INDEXNOW_KEY` in `gen/build.py` (they belong to this site), rebuild, then e.g. GitHub Pages: *Settings → Pages → Source: Deploy from a branch → `main` / root*.
...
| `favicon.svg`, `og-image.png` | Icon and share image; `gen/og_image.py` redraws the latter (rerun on a title change). |
